...
void test_asymetric_performance(Curve curve, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ciphers[i] = ecies_encrypt(keyPairs[i].pemPublicKey, test_message);
    }
    decryption_start = clock();
    for (int i = 0; i < count; i++) {
        messages[i] = ecies_decrypt(keyPairs[i].pemPublicKey, keyPairs[i].pemPrivateKey, ciphers[i]);
    }
    ecdsa_sign_start = clock();
    for (int i = 0; i < count; i++) {
        signatures[i] = ecdsa_sign(keyPairs[i].pemPublicKey, keyPairs[i].pemPrivateKey, ciphers[i], strlen(ciphers[i]));
    }
    ecdsa_verify_start = clock();
    for (int i = 0; i < count; i++) {
        verifications[i] = ecdsa_verify(keyPairs[i].pemPublicKey, ciphers[i], strlen(ciphers[i]), signatures[i]);
    }
    end = clock();
